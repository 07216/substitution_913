function [ simCostOpt ] = c_simulation_cost(y_LDR, L_market,U_market, dist_flag,cvk,cvd,L_capacity, U_capacity, L_demand, U_demand,x,c,s,h,p,hp,pp,N )
...
elseif dist_flag==2
    for kk=1:CapNum
        market_rv = trandn_general( meanm, cvm, Lm, Um, 1 );
        for i=1:N
            sim_capacity(i)=market_rv+trandn_general( (L_capacity(i)+U_capacity(i))/2-meanm, cvk(i), L_capacity(i)-Lm, U_capacity(i)-Um, 1 );
        end
        sim_y=min(y_LDR,x+sim_capacity.');
        for j=1:DeNum
            for i=1:N
