...
function [ E] = b_simulation_piecewise_mean_normal(trun_dex, N, Lk, Uk, trun_r, trun_z, Ld, Ud, trun_rd, trun_zd, cvk, cvd )

meank=(Lk+Uk)/2;
meand=(Ld+Ud)/2;

K=10000; % K is the number of instances
numRow=1+(N+N*(N-1)/2)*trun_rd+N*trun_r; % total number of random variables
MM=zeros(numRow,K);
...
for k=1:K
    ...
    for k1=1:(N-1)
        for k2=(k1+1):N
            d_ex(k1,k2,:)=calculate_lifted_variable(sum(d(k1:k2)),trun_rd,trun_dex(k1,k2,:));
        end
    end
